import { CanActivate, ExecutionContext, Injectable, BadRequestException, HttpStatus, Logger } from '@nestjs/common';
    ...
    for (const idField of this.idFields) {
      ...
      if (Array.isArray(idValue)) {
        for (const id of idValue) {
          this.validateUUID(id, idField);
        }
      } else {
        this.validateUUID(idValue, idField);
      }
    }
  }

  private validateUUID(id: string, idField: string) {
    if (id && !isUUID(id)) {
